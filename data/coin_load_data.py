...
def coin_load_data():
    ...
    # training data 행별로 정규화
    scaler = MinMaxScaler()
    normalized_training_data = coin_training_data.drop(columns=['date']).copy()
    normalized_training_data[:] = scaler.fit_transform(normalized_training_data.values.T).T  # 행별 정규화
    normalized_training_data['date'] = coin_training_data['date'].values

    # chartdata와 trainingdata 병합
    combined_data = pd.concat([coin_chart_data, normalized_training_data.drop(columns=['date'])], axis=1)
    combined_data.index = pd.to_datetime(combined_data.index, errors='coerce')
    combined_data = combined_data.set_index('date')
    combined_data.index = pd.to_datetime(combined_data.index)

    return coin_chart_data, normalized_training_data, combined_data
