...
merged_data.iloc[:, 5:] = scaler.fit_transform(merged_data.iloc[:, 5:].T).T
merged_data.rename(columns={f'close_{i}': f'feature_{i}' for i in range(1, 51)}, inplace=True)
train, test = train_test_split(merged_data, test_size=0.2, random_state=42, shuffle=False)
# %%

# %%
model = PPO.load("../weight/E235.zip")
